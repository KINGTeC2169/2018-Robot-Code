...
import com.team2169.robot.auto.tasks.intake.*;

import java.util.Arrays;

public class LLLAutoCont extends AutoMode {	
/*
...
*/
    ...
    public LLLAutoCont() {
        ...
        addSequential(new ParallelTask(Arrays.asList(new FollowPathInMeters(Paths.LLLPaths.Full),
                new NestedPathTask(Arrays.asList(new ArmConfig(),
                        new ElevatorToSwitch(),
                        new IntakeIdle(),
                        new IntakeClampAction()), 0, 10),

                new NestedPathTask(Arrays.asList(new Task[]{
                        new ElevatorToScaleHigh()
                }), 15, 20),

                new NestedPathTask(Arrays.asList(new IntakeDrop(),
                        new IntakeExhaust(true)), 25, 30),
                new NestedPathTask(Arrays.asList(new ElevatorToGround(),
                        new IntakeNeutral()), 35, 40),
                new NestedPathTask(Arrays.asList(new ArmExtend(),
                        new IntakeUntilHeld()), 45, 50),
                new NestedPathTask(Arrays.asList(new Task[]{
                        new ElevatorToSwitch()
                }), 55, 70),
                new NestedPathTask(Arrays.asList(new IntakeDrop(),
                        new IntakeExhaust(true)), 90, 100))), 15);
